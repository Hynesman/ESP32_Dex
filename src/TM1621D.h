/**
 * \mainpage Shenzhen-Titan-Micro-Elec TM1621D
...
/**
 * \file TM1621D.h
 * \brief A class for dealing with the Holtek TM1621D chip.
...
public:
    /*!
     * \enum Modes
     * Operating modes for the TM1621D.
     */
    enum Modes : uint8_t {
        COMMAND_MODE = 0b10000000, /*!< This is used for sending standard commands. */
        READ_MODE = 0b11000000, /*!< This instructs the TM1621D to prepare for reading the internal RAM. */
        WRITE_MODE = 0b10100000, /*!< This instructs the TM1621D to prepare for writing the internal RAM. */
        READ_MODIFY_WRITE_MODE = 0b10100000, /*!< This instructs the TM1621D to prepare for reading/modifying batch of internal RAM adresses. */
        SPECIAL_MODE = 0b10010000 /*!< This instructs the TM1621D to prepare for executing a special command. */
    };
    
    /*!
     * \enum Commands
     *
     * This is an enum of available commands for the TM1621D.
     *
     */
    ...
    /**
     * \fn TM1621D(uint8_t CSpin, uint8_t RWpin, uint8_t DATApin)
     ...
     */
    TM1621D(uint8_t CSpin, uint8_t RWpin, uint8_t DATApin) : _CS_pin(CSpin), _DATA_pin(DATApin), _RW_pin(RWpin) {};

    /**
     * \fn void begin(void)
     * \brief Init the TM1621D. It inits the control bus. Moreover, it clears the (simulated) ram if \c __TM1621D_READ is defined.
     */
    void begin(void);

    /**
     * \fn void start(void)
     * \brief Start the TM1621D. this means sending the startup sequences to config the projector
     */
    void start(void);
     ...
     * \brief Send bits to the TM1621D.
     * @param data Data to be sent to the TM1621D seen as an array of bits.
     * @param cnt Number of bits to send to the TM1621D.
     ...
     * \brief Reads bits from the TM1621D.
